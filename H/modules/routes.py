    ...
    if form_sign.signup.data and form_sign.validate_on_submit():
        ...
        for i in users:
            if form_sign.username.data == i.username:
                valid_flag=False
                print("Username already taken", i.username)
            if form_sign.email.data == i.email:
                valid_flag=False
                print("Email already taken", i.username)
...
def dashboard():
    ...
    if user_tweet.validate_on_submit():
        ...
        flash("Tweet added to Feed.", 'success')
        return redirect(url_for('dashboard'))
...
def updateInfo():
    ...
    if update.validate_on_submit():
        ...
        if update.profile_bg.data:
            ...
            current_user.backgorund_image = profile_bg_img
